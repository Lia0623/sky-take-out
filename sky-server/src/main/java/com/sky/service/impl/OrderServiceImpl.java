...
//        return vo;
        ...
        // 在这里触发来单提醒
        triggerNewOrderReminder(orderNumber);


        return vo;
    }

    private void triggerNewOrderReminder(String orderNumber) {
        try {
            // 根据订单号查询完整的订单信息
            ...
            if (order != null) {
                ...
                log.info("来单提醒已发送，订单号：{}", orderNumber);
            }
        } catch (Exception e) {
            log.error("发送来单提醒失败，订单号：{}", orderNumber, e);
        }
    }
...
//            orders.setPayStatus(Orders.REFUND);
            ...
            try {
                // 记录退款日志
                log.info("开始模拟微信支付退款，订单号：{}，金额：{}",
                        ordersDB.getNumber(), ordersDB.getAmount());

                // 模拟退款处理时间
                Thread.sleep(100);

                // 模拟退款成功
                log.info("微信支付退款成功，订单号：{}", ordersDB.getNumber());
